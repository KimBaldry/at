# Copyright (C) 2021 Kimberlee Baldry and May-Linn Paulsen
#
...
"at" <-
  function(S=35, T=25, C=0.1, d=1, pHTris=NULL, ETris=NULL, weight, E, volume){
    ...
    # creation of a table p with voltage, volume of acid and temperature (K)
    p <- data.frame(E=E, volume=volume, Tk=Tk)
    z <- p
    
    
    idx = which(E > 0.205 & E < 0.235) #pH 3.5–3 (for Metrohm ecotrode plus), but this part needs refinement to work with different electrodes, some iterative approach to find the right pH interval
